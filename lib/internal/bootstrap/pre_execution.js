...
const { Map, ObjectDefineProperty, SafeWeakMap } = primordials;
...
function prepareMainThreadExecution(expandArgv1 = false) {
	...
	if (getOptionValue('--enable-source-maps')) {
		const { prepareStackTrace } = require('internal/source_map/prepare_stack_trace');
...
function patchProcessObject(expandArgv1) {
	const { patchProcessObject: patchProcessObjectNative } = internalBinding('process_methods');
...
function setupWarningHandler() {
	...
	if (!getOptionValue('--no-warnings') && process.env.NODE_NO_WARNINGS !== '1') {
...
function initializeESMLoader() {
	...
	const { setImportModuleDynamicallyCallback, setInitializeImportMetaObjectCallback } = internalBinding(
		'module_wrap'
	);
